...
public:
    ...
    static Ceres_pose_graph_3d::Constraint3d add_constrain_of_loop(int s_idx /* curr_finished_keyframe_index */, int t_idx /* history_finished_keyframe_index */,
    ...
    void init(std::string path)
    {
        ...
        m_pc_reg.m_max_final_cost = 20000;             // 没用到, loop closure时:  没有通过函数的返回值是0 or 1 来判断是否对齐, 而是通过m_inlier_threshold来判断
        m_pc_reg.m_para_max_speed = 1000.0;            // TODO  in point_cloud_registration.hpp set_ceres_solver_bound() 设置deleta_trans下界和上界, default: 1000.0
        m_pc_reg.m_para_max_angular_rate = 360 * 57.3; // 没用到, loop closure时:  没有通过函数的返回值是0 or 1 来判断是否对齐, 而是通过m_inlier_threshold来判断
    ...
    int find_tranfrom_of_two_mappings(Maps_keyframe<PT_DATA_TYPE> *keyframe_a, // 当前finished keyframe
                                      ...
                                      int if_save = 1,
                                      td::string mapping_save_path = std::string(" "))
    {
        ...
        Eigen::Quaterniond transform_R = Eigen::Quaterniond::Identity(); // 没用到
