    ...
    FUNC_T Eigen::Matrix<DATA_TYPE, 3, 3> get_covmat()
    {
        if (m_covmat_need_update)
        {
            ...
            if (pt_size < 5 || m_if_incremental_update_mean_and_cov == false)
            {
                ...
                // author compute covariance is wired, not consistent with eq3.
...
public:
    ...
    std::vector<std::set<Mapping_cell_ptr>> m_frame_with_cell_index; // 没用到
    ...
    pcl::octree::OctreePointCloudSearch<pcl::PointXYZ> m_octree = pcl::octree::OctreePointCloudSearch<pcl::PointXYZ>(0.0001);           // Octree O 把cells的center用octree存放起来, 以便于快速建立local map kdtree
    pcl::PointCloud<pcl::PointXYZ>::Ptr m_pcl_cells_center = pcl::PointCloud<pcl::PointXYZ>::Ptr(new pcl::PointCloud<pcl::PointXYZ>()); // 点云形式存放每个cell的Cc
...
public:
    ...
    typedef typename std::map<PT_TYPE, Mapping_cell_ptr, PCL_TOOLS::Pt_compare>::iterator Map_pt_cell_it;
#endif
    Map_pt_cell m_map_pt_cell; // 关键帧的hash H
    Map_pt_cell_it m_map_pt_cell_it;
    ...
    pcl::octree::OctreePointCloudSearch<pcl::PointXYZ> m_octree = pcl::octree::OctreePointCloudSearch<pcl::PointXYZ>(0.0001); // 没用到
